...
real4 physical_coords_to_unnormalized_coords(real4 xyz,
  ...
  /* Unnormalized is {0, u, v, w} as computed with fisheye, but we need to
   * do the interpolation to get the correct value */

  int4 uvw_ijk = address_mode((int)floor(unnormalized - K(0.5)), num_points);
  int4 uvw_ijkp1 = uwu_ijk + 1;

  sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;

  real xi = read_imagef(x_grid, sampler, uvw_ijk.x).x;
  real yj = read_imagef(y_grid, sampler, uvw_ijk.y).x;
  real zk = read_imagef(z_grid, sampler, uvw_ijk.z).x;

  real xip1 = read_imagef(x_grid, sampler, uvw_ijkp1.x).x;
  real yjp1 = read_imagef(y_grid, sampler, uvw_ijkp1.y).x;
  real zkp1 = read_imagef(z_grid, sampler, uvw_ijkp1.z).x;

  real4 xyz_ijk = {K(0.0), xi, yj, zk};
  real4 xyz_ijkp1 = {K(0.0), xip1, yjp1, zkp1};

  /* Linear interpolation */
  real4 uvw_interp = uvw_ijk + (xyz - xyz_ijk)/(xyz_ijkp1 - xyz_ijk);
  ...
  printf("Num points %d %d %d\n", num_points.x, num_points.y, num_points.z);
  printf("UVW %d %d %d\n", u, v, w);
  printf("Interp %.16g %.16g %.16g\n", interp.x, interp.y, interp.z);
  *(int*)0 = 0;
